...
called `base_daf_repository` which identifies its parent repository (if any).
...
"""
function complete_daf(leaf::AbstractString, mode::AbstractString = "r"; name::Maybe{AbstractString} = nothing)::Union{DafReader, DafWriter}
    @assert mode in ("r", "r+")
    is_writer = mode == "r+"
...
Open either a [`FilesDaf`](@ref) or an [`H5df`](@ref). If the `path` ends with `.h5df` or contains `.h5df//` followed by
a group path, then it opens an HDF5 file (or a group in one). Otherwise, it opens a files `Daf`.
"""
function open_daf(path::AbstractString, mode::AbstractString = "r"; name::Maybe{AbstractString} = nothing)::Union{DafReader, DafWriter}
    if endswith(path, ".h5df") || occursin(".h5df//", path)
