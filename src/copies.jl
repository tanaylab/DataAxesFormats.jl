...
function copy_vector!(;
    ...
    if relation == nothing
        relation = verify_axis(
            ...
            axis;
            allow_missing = false,
            allow_source_subset = empty != nothing,
        )
    ...
    verify_subset(source.name, axis, destination.name, reaxis, empty != nothing)
...
function copy_matrix!(;
    ...
    if columns_relation == :source_is_subset
        verify_subset(source.name, columns_axis, destination.name, columns_reaxis, empty != nothing)
    end

    if rows_relation == :source_is_subset
        verify_subset(source.name, rows_axis, destination.name, rows_reaxis, empty != nothing)
    end
...
function copy_all!(;
    ...
    axis_relations = verify_axes(destination, source; allow_source_subset = (empty != nothing))
...
function verify_axes(destination::DafWriter, source::DafReader; allow_source_subset::Bool)::Dict{AbstractString, Symbol}
    ...
    for axis in axis_names(source)
        axis_relations[axis] = verify_axis(
            ...
            axis;
            allow_missing = true,
            allow_source_subset = allow_source_subset,
        )
    end
...
function verify_axis(
    ...
    if source_entries < destination_entries
        verify_subset(source_daf.name, source_axis, destination_daf.name, destination_axis, allow_source_subset)
...
function verify_subset(
    ...
    if !allow_subset
        error(
            "missing entries source the axis: $(source_axis)\n" *
            "of the source daf data: $(source_name)\n" *
            "which are needed for the axis: $(destination_axis)\n" *
